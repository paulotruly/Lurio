...
async function main(canvas) {
    ...
    const lurio = entityFactory.lurio();
    makePlayer(lurio, "LURIO");
    window.lurio = lurio;

    const inputRouter = setupKeyboard(window);
    inputRouter.addReceiver(lurio);
    ...
    async function runLevel(name) {
        ...
        /**level.events.listen(Level.EVENT_TRIGGER, (spec, trigger, touches) => {
            if (spec.type === "goto") {
                for (const _ of findPlayers(touches)) { (Trigger para mudar de fase)
                    runLevel(spec.name);
                    return;
                }
            }
        ...
        lurio.pos.set(0, 0);
        level.entities.add(lurio);

        const playerEnv = createPlayerEnv(lurio);
        ...
        /**level.comp.layers.push(createCollisionLayer(level)); (ativa o visual do layer de colisão das entidades**/
